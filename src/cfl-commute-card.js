...
class CflCommuteCard extends LitElement {
  ...
  _getTrainsFromIndividualSensors(hass, entityId) {
    ...
    const patterns = [
      ...
      `sensor.${baseName}_train`,
      `sensor.${baseName.replace(/_/g, '-')}_train_`,
      `sensor.${baseName.replace(/_/g, '')}_train_`,
    ];

    let trainSensors = [];
    for (const pattern of patterns) {
      const found = Object.keys(hass.states).filter(eid =>
        eid.startsWith(pattern)
      );
      if (found.length > 0) {
        trainSensors = found;
        break;
...
window.customCards.push({
  ...
  description: 'Display CFL train departures in a station departure board style',
  preview: true,
