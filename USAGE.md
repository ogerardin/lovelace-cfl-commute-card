# CFL Commute Card - Usage Guide

This card provides a CFL station-style departure board view. For other view modes (full, compact, next-only), use the [lovelace-my-rail-commute-card](https://github.com/adamf83/lovelace-my-rail-commute-card) by adamf83, which is compatible with CFL integration data.

## Setup

### Configure your commutes
Install as described in https://github.com/ogerardin/ha-cfl-commute/blob/main/README.md

Create at least one commute. This will create sensors like:
- `sensor.your_commute_name_summary`
- `sensor.your_commute_name_status`
- `sensor.your_commute_name_next_train`

### Add the Card to Your Dashboard

#### Visual Editor
1. Enter **Edit Dashboard** mode
2. Click **Add Card**
3. Search for "CFL Commute Card"
4. Select your summary entity
5. Configure options as desired
6. Save

#### YAML Configuration

```yaml
type: custom:cfl-commute-card
entity: sensor.your_commute_summary
```

## Configuration Options

| Option | Type | Default | Description |
|--------|------|---------|-------------|
| `entity` | string | Required | Your CFL Commute summary sensor |
| `title` | string | '' | Card title (shown in empty/loading states) |
| `status_entity` | string | '' | Status sensor for disruption banner. Auto-discovered if not set |
| `hide_on_time_trains` | boolean | false | Hide trains that are on time |
| `min_delay_to_show` | number | 0 | Minimum delay in minutes to show a train (0 = show all) |
| `only_show_disrupted` | boolean | false | Only show trains with disruptions |
| `auto_refresh` | boolean | true | Auto-refresh data |
| `refresh_interval` | number | 60 | Refresh interval in seconds |
| `tap_action` | object | more-info | Tap action: more-info, url, navigate, none |
| `hold_action` | object | refresh | Hold action: refresh, more-info, none |
| `show_route` | boolean | true | Show origin → destination route in the departure board header |
| `calling_points_scroll_interval` | number | 5000 | Pause interval (ms) between calling points line shifts |
| `show_history_panel` | boolean | false | Show collapsible reliability history panel below the departure board |
| `history_days` | number | 7 | Number of days of historical data to display (used with show_history_panel) |
| `colors` | object | {} | Custom color overrides (on_time, minor_delay, major_delay, cancelled) |

### Advanced Example

```yaml
type: custom:cfl-commute-card
entity: sensor.morning_commute_summary
status_entity: sensor.morning_commute_status
show_route: true
hide_on_time_trains: false
min_delay_to_show: 0
calling_points_scroll_interval: 5000
show_history_panel: true
history_days: 7
tap_action:
  action: url
  url_path: https://www.cfl.lu
```

## Troubleshooting

### Card not showing / Visual editor not supported
- Refresh browser cache (Ctrl+Shift+R)
- Verify resource is added correctly in Settings → Dashboards → Resources
- Check browser console for errors

### No trains displayed
- Verify CFL Commute integration is working
- Check entity exists in Developer Tools → States
- Ensure sensor has data in `all_trains` attribute

### Platform shows [object Object]
- Update to the latest card version (1.0.7+) and integration version (1.8.1+)
- The integration now normalizes platform to a string

### Board view shows commute destination instead of train destination
- Update the CFL Commute integration to v1.8.1+
- The `direction` field is required for per-train destinations

### Board view missing calling points
- Update the CFL Commute integration to v1.8.1+
- Calling points are now included in the summary sensor's `all_trains` attribute

### Want full, compact, or next-only views?
- Use the [lovelace-my-rail-commute-card](https://github.com/adamf83/lovelace-my-rail-commute-card) which shares the same data format

## Need Help?

- [Open an issue](https://github.com/ogerardin/lovelace-cfl-commute-card/issues)